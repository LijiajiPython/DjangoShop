...
def add_goods(request):
    ...
    if request.method=="POST":
        ...
        goods.save()
        return HttpResponseRedirect("/AppShop/list_goods")
    return render(request,"appshop/add_goods.html")
...
def list_goods(request):
    ...
    if keywords:
        good_list=Goods.objects.filter(store_id=storeid,goods_name__contains=keywords)
    else:
        good_list=Goods.objects.filter(store_id=storeid)
...
def edit_goods(request,goods_id):
    ...
    if request.method=="POST":
        ...
        if goods_image:
            goods.goods_image=goods_image
        goods.save()
        return HttpResponseRedirect("/AppShop/goods/%s/"%goods_id)
